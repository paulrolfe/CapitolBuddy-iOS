...
- (void)setCenterViewController:(UIViewController *)centerViewController {
    ...
    [((UIViewController *)_centerViewController) view].frame = (CGRect){.origin = origin, .size=self.view.bounds.size};
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        //if it's in the center view.
        if([gestureRecognizer.view isEqual:[self.centerViewController view]]){
            //if it's 50 pixels away or less, then the centerview can be allowed.
            if([touch locationInView:[self.centerViewController view]].x<50){
...
shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {    
	return self.recognizeMultipleGestures;
}
