...
{    
    ...
    NSString *jsonObjectString = [self base64forData:receiptData];
    //NSString *jsonObjectString = [self encodeBase64:(uint8_t *)receiptData.bytes length:receiptData.length];
...
#warning Change this from sandbox before submitting to app store
...
- (BOOL)isTransactionAndItsReceiptValid:(SKPaymentTransaction *)transaction
{
    ...
    //[[NSUserDefaults standardUserDefaults] setObject:purchaseDate forKey:productID];
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    if (0 != verifyReceiptStatus && 21006 != verifyReceiptStatus && 21007!=verifyReceiptStatus)
    {
        return NO;
    }
